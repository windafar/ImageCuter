...
namespace ImageBasic
{
    static public class BasicMethodClass
    {
        ...
        /// <returns></returns>
        public static Bitmap CutImage(Bitmap srcbit, int startx, int starty, int distwidth, int distheight)
        {
            if (srcbit == null)
            {
                return null;
            ...
            try
            {
                Bitmap bmpOut = new Bitmap(distwidth, distheight, PixelFormat.Format24bppRgb);
                Graphics g = Graphics.FromImage(bmpOut);
                g.DrawImage(srcbit, new Rectangle(0, 0, distwidth, distheight), new Rectangle(startx, starty, distwidth, distheight), GraphicsUnit.Pixel);
                g.Dispose();
                return bmpOut;
            }
            catch(Exception ex)
        ...
        static public bool MakeThumbnail(MemoryStream originalStream, Stream SaveStream, double width, double height, string mode, string type, int size = int.MaxValue)
        {
            if (SaveStream == null) SaveStream = new MemoryStream();
            System.Drawing.Image originalImage;
            try
            {
                MemoryStream original_ms = originalStream;
                originalImage = new Bitmap(original_ms);
                original_ms.Dispose();
                if (width < 0 || height < 0) return false;
            }
